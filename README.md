...
2. Run `submit_moo_infra_bridges2.sh` for multiple random seed replicate trials. Each submisssion uses 32 nodes on the RM partition on Bridges-2. I ran 4 seeds. For each, begin with numFEPrevious=0 (a fresh start for MOEA), and run for 48 hours.
...
1. Once the optimization and reevaluation steps have been completed, the remaining analysis and figure generation can be done on a local computer after results have been copied there. 3
